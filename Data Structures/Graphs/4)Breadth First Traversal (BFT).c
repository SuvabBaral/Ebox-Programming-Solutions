...
void Bfs(int **graph, int *size, int presentVertex,int *visited, int V)
{
    ...
    Enqueue(queue,presentVertex);
    printf("%d ",presentVertex);
    int i,j;
    for(i=queue->elements[queue->front];queue->size!=0;i=queue->elements[queue->front]){
        for(j=0;j<size[i];j++){
        if(visited[graph[i][j]]==0){
                visited[graph[i][j]]=1;
                Enqueue(queue,graph[i][j]);
                printf("%d ",graph[i][j]);
